...
if __name__=='__main__':
    ...
    try:
        request_client=DoRequest(Read_WEB_UI_Config().web_ui_config.selenium_hub)
    ...
    for current_browser in Read_WEB_UI_Config().web_ui_config.test_browsers:
        ...
        # 执行pytest前的参数准备
        pytest_execute_params=['-c', 'config/pytest.ini', '--rootdir=.', '-v', '--alluredir', 'output/web_ui/'+current_browser+'/report_data/','-n',Read_WEB_UI_Config().web_ui_config.test_workers,'--dist','loadfile']
        # 判断目录参数
        dir = 'cases/web_ui/'
        if args.dir:
            dir=args.dir
