...
def start_app_device_test(index,device_info,keyword,dir,markexpr,capture,reruns,lf,clr):
    ...
    try:
        request_client = DoRequest('http://'+device_info['server_ip']+':%s/wd/hub'%device_info['server_port'].strip())
        httpResponseResult = request_client.get('/status')
    ...
    for desired_capabilities in a_devices_desired_capabilities:
        ...
        # 执行pytest前的参数准备
        pytest_execute_params = ['-c', 'config/pytest.ini', '--rootdir=.', '-v', '--alluredir', 'output/app_ui/%s/%s/report_data/'%(device_info['device_desc'],desired_capabilities_desc)]
        # 判断目录参数
        if not dir:
            dir = 'cases/app_ui/'
        # 判断关键字参数
...
if __name__=='__main__':
    ...
    if not args.test_type:
        sys.exit('请指定测试类型，查看帮助:python -m scripts.runners.run_app_ui_test --help')
    ...
    if test_type=='phone':
        if not devices_info_file:
            sys.exit('请指定多设备并行信息文件，查看帮助:python -m scripts.runners.run_app_ui_test --help')
        ...
        # 执行pytest前的参数准备
        pytest_execute_params = ['-c', 'config/pytest.ini', '--rootdir=.', '-v', '--alluredir', 'output/app_ui/windows/report_data/']
        # 判断目录参数
        if not dir:
            dir = 'cases/app_ui/'
        # 判断关键字参数
