#-*- coding:utf8 -*-
"""作用：定义demoProject login相关的自动化测试用例。"""
...
class TestLogin:
    ...
    def setup_class(self):
        self._api_demo_project_client=API_DemoProject_Client()
        self._login_path='/horizon/auth/login/'

    def test_get_index(self):
        httpResponse=self._api_demo_project_client.doRequest.get(self._login_path)
        assert_that(200).is_equal_to(httpResponse.status_code)

    @pytest.mark.search_kw
    def test_search_kw(self):
        params={'wd':'apitest'}
        httpResponse = self._api_demo_project_client.doRequest.get(self._login_path,params)
        assert_that(200).is_equal_to(httpResponse.status_code)
