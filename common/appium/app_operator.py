...
from pojo.element_info import ElementInfo
...
class _DeferredTouchAction:
    """封装可延迟执行的单个触摸动作，兼容旧的多动作调用方式。"""

    def __init__(self, performer, point=None, duration=0, count=1):
...
class AppOperator:
    """
    类中的element参数可以有appium.webdriver.webelement.WebElement和pojo.element_info.ElementInfo类型
    """
    ...
    def click_web_element(self,element):
        ...
        webElement=self._change_element_to_webElement_type(element)
        if webElement:
            ActionChains(self._driver).move_to_element(webElement).click().perform()
    ...
    def get_captcha(self,element,language='eng'):
        ...
        """
        # 识别图片验证码
        from common.captcha_recognition_tool import CaptchaRecognitionTool
    ...
    def touch_move_to(self,start_x,start_y,end_x,end_y,long_press=True,duration=0):
        ...
        :return:
        """
        hold_duration = duration if long_press else 0
        self._perform_touch_gesture(start_x, start_y, end_x, end_y, hold_duration)

    def _perform_touch_gesture(self, start_x, start_y, end_x, end_y, duration=0):
        """使用W3C Pointer Actions执行触摸拖动，替代已移除的TouchAction。"""
        touch_input = PointerInput('touch', 'finger')
        actions = ActionBuilder(self._driver, mouse=touch_input)
        actions.pointer_action.move_to_location(int(start_x), int(start_y))
        actions.pointer_action.pointer_down()
        if duration:
            actions.pointer_action.pause(max(duration, 0) / 1000)
        actions.pointer_action.move_to_location(int(end_x), int(end_y))
        actions.pointer_action.release()
        actions.perform()

    @staticmethod
    def _element_touch_point(web_element, xoffset=None, yoffset=None):
        """计算元素中心或相对偏移对应的触摸坐标。"""
        rect = web_element.rect
        x = rect['x'] + (rect['width'] / 2 if xoffset is None else xoffset)
        y = rect['y'] + (rect['height'] / 2 if yoffset is None else yoffset)
        return int(x), int(y)

    def _tap_coordinates(self, x, y, count):
        """在指定坐标执行一次或多次触摸点击。"""
    ...
    def touch_tap(self,element,xoffset=None,yoffset=None,count=1,is_perfrom=True):
        ...
        webElement=self._change_element_to_webElement_type(element)
        if webElement:
            x, y = self._element_touch_point(webElement, xoffset, yoffset)
            actions = _DeferredTouchAction(
                lambda: self._tap_coordinates(x, y, count),
                point=(x, y),
                count=count,
            )
            if is_perfrom:
    ...
    def touch_long_press(self,element,xoffset=None,yoffset=None,duration_sconds=10,is_perfrom=True):
        ...
        if webElement:
            x, y = self._element_touch_point(webElement, xoffset, yoffset)
            duration = duration_sconds * 1000
            actions = _DeferredTouchAction(
                lambda: self._perform_touch_gesture(x, y, x, y, duration),
                point=(x, y),
                duration=duration,
            )
            if is_perfrom:
    ...
    def multi_touch_actions_perform(self,touch_actions):
        ...
        has_composable_action = False
        for index, actions in enumerate(touch_actions):
            if not isinstance(actions, _DeferredTouchAction) or actions.point is None:
                actions.perform()
                continue
            has_composable_action = True
            pointer = action_builder.add_pointer_input('touch', 'finger%s' % index)
            x, y = actions.point
            for _ in range(actions.count):
                pointer.create_pointer_move(duration=0, x=x, y=y, origin='viewport')
                pointer.create_pointer_down(button=0)
                if actions.duration:
                    pointer.create_pause(actions.duration / 1000)
                pointer.create_pointer_up(0)
        if has_composable_action:
    ...
    def getSubElement(self,parent_element,sub_elementInfo):
        ...
        :param sub_elementInfo: 子元素,只能提供pojo.element_info.ElementInfo类型
        :return:
        """
